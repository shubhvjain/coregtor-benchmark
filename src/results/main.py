...
def build_parser():
    ...
    parser.add_argument("--input", required=True, metavar="PATH", help="Path to the JSON input file.")
    parser.add_argument(
        "--id",
        required=True,
        metavar="ID",
    ...
    parser.add_argument("--n-jobs", type=int, default=-1, help="Number of parallel jobs. Defaults to -1 (all cores).")
    parser.add_argument("--rerun", action="store_true", default=False, help="Force rerun even if output already exists.")
    parser.add_argument("--verbose", action="store_true", default=False, help="Enable verbose output.")
    parser.add_argument("--dry-run", action="store_true", default=False, help="Load files but skip execution.")
...
def main():
    ...
    if result_type not in METHOD_REGISTRY:
        print(f"[error] unknown type '{result_type}'. Registered types: {', '.join(METHOD_REGISTRY.keys())}", file=sys.stderr)
        sys.exit(1)

    fn = METHOD_REGISTRY[result_type]

    if args.dry_run:
        print(f"[dry-run] would call '{result_type}' for id '{args.id}' with {len(env)} env key(s).")
        return
