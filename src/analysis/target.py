...
def _generate_save_stats_diagram(df, folder_path):
    ...
    fig, axes = plt.subplots(
        ...
        gridspec_kw={"wspace": 0.35, "hspace": 0.5} # Slightly increased hspace for rotated labels
    )
    ...
    sns.barplot(data=df, x="dataset_abbr", y="dataset_sample_size", ax=ax_list[0], color=default_bar_color)
    ...
    for i, ax in enumerate(ax_list):
        ...
        # Format X-axis ticks
        ax.tick_params(axis="x", labelbottom=should_show_label, rotation=45, labelsize=7) 
    ...
    fig.savefig(folder_path / "gene_frequency_histogram.pdf", bbox_inches="tight")
    fig.savefig(folder_path / "gene_frequency_histogram.svg", bbox_inches="tight")
    plt.close(fig)
...
def _generate_save_ps_diagram(df, folder_path):
    # 'paper' context ensures labels and fonts are appropriately scaled for publication
    sns.set_theme(style="whitegrid", context="paper", font_scale=1.0)
